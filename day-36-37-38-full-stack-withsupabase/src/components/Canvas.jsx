...
export const Canvas = () => {
  ...
  const activeConnections = connections.filter(conn => activeCards.some(c => c.id === conn.source) && activeCards.some(c => c.id === conn.target));
  ...
  useEffect(() => {
    ...
    const vpDraggable = Draggable.create(wrapperRef.current, {
      ...
      onPress: function(e) {
        ...
        const target = e.target?.nodeType === 3 ? e.target.parentNode : e.target;
        if (selectedCardIds.length > 0 || target?.closest?.('.card-node') || target?.closest?.('.port')) {
          this.endDrag(e);
        }
      },
      onDrag: function() {
  ...
  const calculateBezier = (p1, p2, port1, port2) => {
    ...
    const getControlPoint = (p, port) => {
      let cp = { ...p };
      switch(port) {
        case 'top': cp.y -= offset; break;
        case 'right': cp.x += offset; break;
        case 'bottom': cp.y += offset; break;
        case 'left': cp.x -= offset; break;
        case 'center': break;
      }
      return cp;
    };
      ...
      if (node) {
        ...
        const d = Draggable.create(node, {
          ...
          clickableTest: function(e) {
            ...
            const target = e.target?.nodeType === 3 ? e.target.parentNode : e.target;
            if (target?.closest?.('input, textarea, button, select, [contenteditable="true"], .port, [data-resize-handle]')) return true;
  ...
  const handlePointerMove = (e) => {
    if (activeTool === 'connect' && currentConnectingRef.current) {
      ...
      if (activeDrawingTool === 'eraser') {
        ...
        const intersectingIds = drawings.filter(d => {
  ...
  const handlePointerUp = (e) => {
    ...
    if (activeTool === 'cursor' && isLassoing.current && lassoRect) {
      ...
      activeCards.forEach(card => {
        ...
        if (
          card.x < maxX && card.x + cardW > minX &&
          card.y < maxY && card.y + cardH > minY
        ) {
          if (!store.selectedCardIds.includes(card.id)) {
  ...
  return (
    ...
    >
      ...
      {selectedCardIds.length > 1 && (
        ...
        >
          <span className="text-sm font-semibold text-gray-500">{selectedCardIds.length} selected</span>
