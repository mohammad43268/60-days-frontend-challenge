...
export const LandingPage = () => {
  ...
  const handleEmailLogin = async (e) => {
    ...
    // Optimistic UI - instantly show success screen
    setAuthSuccess(true);
    setAuthMessage('');
    setAuthLoading(true);
    ...
    supabase.auth.signInWithOtp({ email }).then(({ error }) => {
      ...
      if (error) {
        // If it fails (e.g., rate limit), revert to the form and show error
          ...
          <div className="relative w-full max-w-md bg-[#0A0A0B] border border-white/10 rounded-2xl p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            ...
            {authSuccess ? (
              <div className="flex flex-col items-center justify-center py-6 text-center">
                <div className="w-16 h-16 rounded-full bg-[#F97316]/20 flex items-center justify-center mb-6">
                  <svg className="w-8 h-8 text-[#F97316]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <h3 className="text-xl font-bold text-white mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Magic Link Sent!</h3>
                <p className="text-gray-400 text-sm mb-6 max-w-[280px] mx-auto">
                  We've sent a secure login link to <br/> <span className="text-white font-medium">{email}</span>. <br/><br/> Please check your inbox (and spam folder) to securely access Zaforge.
                </p>
                <button 
                  onClick={() => {
                  ...
                  className="w-full bg-white/5 border border-white/10 text-white font-medium py-3 rounded-lg hover:bg-white/10 transition-colors uppercase tracking-widest text-xs"
                >
                  Close & Wait
                </button>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
                  ...
                  <button 
                    ...
                    className="w-full bg-[#F97316] text-black font-bold py-3 rounded-lg hover:scale-[1.02] transition-transform disabled:opacity-50 disabled:hover:scale-100"
                  >
                ...
                {authMessage && (
                  <div className="text-center text-sm font-mono text-[#F97316]">
                    {authMessage}
                  </div>
                )}
                ...
                <button 
                  ...
                  disabled={authLoading}
                  className="w-full flex items-center justify-center gap-3 bg-white/5 border border-white/10 text-white font-medium py-3 rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50"
                >
