...
export const CommandPalette = () => {
  ...
  const actions = [
    {
      id: 'view-canvas',
      label: 'Switch to Spatial Canvas',
      icon: Layout,
      action: () => setViewMode('canvas'),
    },
    {
      id: 'view-table',
      label: 'Switch to SaaS Table',
      icon: TableProperties,
      action: () => setViewMode('table'),
    },
    {
      id: 'view-gantt',
      label: 'Switch to Gantt Timeline',
      icon: CalendarDays,
      action: () => setViewMode('gantt'),
    },
    {
      id: 'template-study',
      label: 'Load Study Template',
      icon: Lightbulb,
      action: () => loadTemplate('study'),
    },
    {
      id: 'template-coding',
      label: 'Load Coding Template',
      icon: Lightbulb,
      action: () => loadTemplate('coding'),
    },
    {
      id: 'template-planning',
      label: 'Load Planning Template',
      icon: Lightbulb,
      action: () => loadTemplate('planning'),
    },
            ...
            <div className="py-2">
              {results.map((res, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center mr-3 ${isSelected ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500'}`}
                    >
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <div
                        className={`text-xs truncate mt-0.5 ${isSelected ? 'text-orange-400' : 'text-gray-400'}`}
                      >
                        {res.type}
                      </div>
                    </div>
                    {isSelected && (
                      <div className="text-xs text-orange-500 font-medium">↵ to select</div>
                    )}
                  </button>
                );
              })}
