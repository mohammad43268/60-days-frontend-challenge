    ...
    <div className="w-full h-full bg-[#161618] text-gray-200 p-8 pt-24 overflow-auto">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white drop-shadow-md">Node Directory</h1>
                        ...
                        <div className="flex flex-col gap-2 max-w-[300px]">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-black/40 border border-white/5 h-1.5 rounded-full overflow-hidden">
                              <div className="bg-orange-500 h-full rounded-full transition-all shadow-[0_0_10px_rgba(249,115,22,0.8)]" style={{ width: `${progress}%` }} />
                            </div>
                    ...
                    <td className="px-6 py-4">
                      {card.metadata?.assignee ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-gray-300">{card.metadata.assignee}</span>
                        </div>
                      ) : (
                        <span className="text-sm text-gray-500 flex items-center gap-1.5"><User className="w-4 h-4" /> Unassigned</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      {card.metadata?.priority ? (
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold ${
                          card.metadata.priority === 'Urgent' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                          card.metadata.priority === 'High' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                    ...
                    <td className="px-6 py-4 text-right">
                      {(card.metadata?.startDate || card.metadata?.endDate) ? (
                        <div className="flex items-center justify-end gap-2 text-sm text-gray-400 font-medium">
                          ...
                          {card.metadata?.startDate || 'TBD'} <ArrowRight className="w-3 h-3 text-gray-600" /> {card.metadata?.endDate || 'TBD'}
                        </div>
                      ) : (
