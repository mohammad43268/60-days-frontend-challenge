    ...
    <div className="w-full h-full bg-[#161618] text-gray-200 p-8 pt-24 overflow-auto flex gap-6">
      ...
      <div className="flex-1 flex flex-col max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white drop-shadow-md">Timeline</h1>
            <p className="text-sm text-gray-400 mt-1">{monthName} {year} Schedule</p>
          </div>
        ...
        <div className="bg-[#2A2A35]/40 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-2xl flex-1 flex flex-col">
          ...
          <div className="flex border-b border-white/10 bg-black/40">
            ...
            <div className="flex-grow flex relative">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex-1 border-r border-white/5 last:border-r-0 p-2 text-[10px] font-medium text-gray-500">
          ...
          <div className="relative flex-1 p-2 space-y-2 overflow-y-auto custom-scrollbar">
            ...
            <div className="absolute top-0 bottom-0 left-48 right-0 flex pointer-events-none z-0">
               {Array.from({ length: 5 }).map((_, i) => (
                 <div key={i} className="flex-1 border-r border-white/5 border-dashed last:border-r-0" />
               ))}
            </div>
            ...
            {cards.map(card => {
              ...
              return (
                <div key={card.id} className="relative flex items-center h-14 bg-white/5 hover:bg-white/10 rounded-lg transition-colors z-10 group border border-transparent hover:border-white/10">
                  <div className="w-48 flex-shrink-0 truncate px-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-gray-500 mt-0.5">{card.metadata?.assignee || 'Unassigned'}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex-grow relative h-full flex items-center pr-4">
                    <div 
                      className={`h-8 rounded-lg flex items-center px-3 text-xs font-bold truncate shadow-sm transition-transform hover:scale-[1.02] cursor-pointer ${
                        card.type === 'task' ? 'bg-gradient-to-r from-orange-400 to-amber-500 text-white border border-orange-500/50 shadow-[0_0_10px_rgba(249,115,22,0.4)]' :
      ...
      <div className="w-80 flex flex-col">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white drop-shadow-md">Schema</h1>
        ...
        <div className="bg-[#2A2A35]/40 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          ...
          {connections.length === 0 ? (
            ...
            connections.map(conn => {
              ...
              return (
                <div key={conn.id} className="bg-black/40 rounded-xl p-3 border border-white/5 hover:border-orange-500/50 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${
                      conn.type === 'blocks' ? 'bg-red-500/20 text-red-400' :
                      conn.type === 'depends_on' ? 'bg-amber-500/20 text-amber-400' :
                      conn.type === 'owned_by' ? 'bg-green-500/20 text-green-400' :
